...
/*
			...
			headerGenerateBackgroundColor: false,
			fullscreen: false,
			autoSlide: true,
			arrowControl: false,
			thumbnail: true,
...
*/
		...
		this.calculateSizes = function(){
			...
			if ( _.opts.fullscreen ){
				...
				if(this.opts.thumbnail){
					this.thumbnailWrapperHeight = $(window).width();
					this.thumbnailWrapperWidth = $(window).height();
				...
				if( this.opts.thumbnailHeight === 'auto' || !this.thumbnailWrapperScrollable){
					this.thumbnailHeight = ( ( this.slideHeight - this.opts.thumbnailBorderSize ) / this.numberOfSlides  );
				}
				else {
			...
			this.thumbnailWidth = ( ( this.slideWidth / this.numberOfSlides ) - this.opts.thumbnailBorderSize );
		};
		...
		this.createSlides = function(backupText){
			...
			$(obj).children('li').each(function(i, el){
				...
				$(el).children('div').children('p').css({
